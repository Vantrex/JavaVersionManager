...
import de.vantrex.jdkswitcher.util.CommandRunner;
import de.vantrex.jdkswitcher.util.JavaPathMatcher;
import de.vantrex.jdkswitcher.util.ProfileLine;
...
public class LinuxPlatform implements IPlatform {
    ...
    public void switchToDefault() {
        final JDKSwitcherConfig config = this.configurationProvider.getConfig();
        try {
            final List<ProfileLine> lines = this.getCurrentDotProfileContent();
            final List<ProfileLine> toEdit = this.findLinesToEditInDotProfileFile(lines);
...
//        CommandRunner.handleBashCommand("bash", new String[]{"-c", "export", javaHomeEnv});
 //       CommandRunner.handleBashCommand("bash", new String[]{"-c", "export", "PATH=" + "\"$JAVA_HOME" + File.separator + "bin:$PATH\""});
    ...
    private void exportDefaultJavaHome(final String javaHome) throws IOException {
        ...
        profileLines.add(new ProfileLine(currentIndex, "# Added by Java Version Manager (JdkSwitcher)"));
